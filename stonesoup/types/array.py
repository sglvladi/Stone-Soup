...
class Matrix(np.ndarray):
    ...
    def __array_ufunc__(self, ufunc, method, *inputs, **kwargs):
        if ufunc in (np.isfinite, np.matmul):
            ...
            inputs = [np.asarray(input_, dtype=float) if isinstance(input_, Matrix) else input_
...
class StateVectors(Matrix):
        ...
        if state_vectors.dtype != np.object_:
            ...
            for dim, row in enumerate(state_vectors):
                ...
                if hasattr(type_, 'average'):
                    ...
                    state_vector[dim, 0] = type_(np.average(np.asarray(row, dtype=float), weights=weights))
        else:
            return NotImplemented
    ...
    def _cov(state_vectors, y=None, rowvar=True, bias=False, ddof=None, fweights=None,
        ...
        if state_vectors.dtype != np.object_:
            ...
            X = np.asarray(state_vectors - avg, dtype=float)
            if aweights is None:
                X_T = X.T
            else:
                X_T = (X*np.asarray(aweights, dtype=float)).T
            cov = X @ X_T.conj()
