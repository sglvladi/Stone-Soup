"""
multi-sonar-mfa-fuse-direct-bias.py
...
- The configuration is the same as in multi-sonar-mfa-fuse-direct.py, but with the addition of
...
from stonesoup.models.transition.linear import (CombinedLinearGaussianTransitionModel,
                                                ConstantVelocity, ConstantTurn, RandomWalk,
                                                NthDerivativeDecay)
...
# Fusion Tracker
transition_model_bias = CombinedLinearGaussianTransitionModel([ConstantVelocity(0.05),
                                                               ConstantVelocity(0.05),
...
hypothesiser2 = PDAHypothesiserNoPrediction(predictor=None,
                                            updater=two_state_updater,
                                            clutter_spatial_density=Probability(-40, log_value=True),
                                            prob_detect=Probability(prob_detect),
                                            prob_gate=Probability(0.99))
hypothesiser2 = DistanceGater(hypothesiser2, Mahalanobis(), 20)
hypothesiser2 = MFAHypothesiser(hypothesiser2)
fuse_associator2 = MFADataAssociator(hypothesiser2, slide_window=slide_window)

fuse_tracker = FuseTracker(initiator=fuse_initiator, predictor=two_state_predictor,
                           updater=two_state_updater, associator=fuse_associator2,
