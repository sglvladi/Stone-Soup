...
from ..types.state import State, GaussianState, TaggedWeightedGaussianState
from ..types.mixture import GaussianMixture
from ..types.track import Track
from ..types.update import (GaussianStateUpdate, ParticleStateUpdate, Update,
                            GaussianMixtureUpdate, TaggedWeightedGaussianStateUpdate)
...
class SimpleMeasurementInitiatorMixture(GaussianInitiator):
    """Initiator that maps measurement space to state space
    ...
    def initiate(self, detections, timestamp, **kwargs):
        ...
        for detection in detections:
            ...
            #     timestamp=detection.timestamp)
            # ]))
            tracks.add(Track([GaussianMixture(components=[
                TaggedWeightedGaussianStateUpdate(prior_state_vector + state_vector,
                                                  C0,
...
class MultiMeasurementInitiatorMixture(MultiMeasurementInitiator):
    """Multi-measurement initiator where output tracks' state is a Gaussian Mixture"""

    def initiate(self, detections, timestamp, **kwargs):
        sure_tracks = super().initiate(detections, timestamp, **kwargs)
...
class MixtureMultiMeasurementInitiator(GaussianInitiator):
    """Multi-measurement initiator.
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.holding_tracks = set()
        if self.initiator is None:
            self.initiator = SimpleMeasurementInitiatorMixture(self.prior_state, self.measurement_model)
