...
from stonesoup.reader.blue import BlueDetectionReaderMatlab, BlueDetectionReaderFile, BlueMultiDetectionReaderFile
...
def plot_truth(target_pos_hit, ax, i):
    ax.plot(target_pos_hit[0, 0:i], target_pos_hit[1, 0:i], target_pos_hit[2, 0:i], 'k+-')
...
detector = BlueMultiDetectionReaderFile(path=file_path)
...
for i, (timestamp, detections) in enumerate(detector):
    ...
    for j in range(3):
        plot_truth(detector._truthdata.target_pos_hit[j], ax, i)
...
plot_coords(track, detector._truthdata)
plot_bias(track, detector._truthdata)