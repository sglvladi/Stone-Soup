...
from ...custom.graph import get_xy_from_range_edge, get_xy_from_range_endnodes, CustomDiGraph
...
class AimpointMeasurementModel(NonLinearGaussianMeasurement):
    ...
    def function(self, state, noise=None, **kwargs):
        ...
        r = state_vectors[0, :]
        e = state_vectors[2, :]
        p1 = state_vectors[[7, 8], :]
        p2 = state_vectors[[5, 6], :]

        # Transform range and edge to xy
        xy = get_xy_from_range_endnodes(r, p1, p2)
        # xy = get_xy_from_sv(state_vectors, self.spaths, self.graph)
        return xy + noise
