...
def prepare_tracks(dataset, gnd, tracks):
    if dataset in ['Sim1', 'Real']:
        # Associate tracks to ground truth and extract the track that is associated to the ground truth
...
def prepare_tracks_fuse(dataset, fuse_timestamps, all_gnd, fused_tracks, local_tracks):
    """Helper function to prepare tracks for both fusion and local trackers.

    For Dataset 'Sim1' and 'Real', the function associates the tracks to ground truth and extracts the track that is
    associated to the ground truth. For Dataset 'Sim2', the function does not perform any association.

    Parameters
    ----------
    dataset : str
        The dataset to prepare tracks for. Either 'Sim1', 'Sim2', or 'Real'.
    fuse_timestamps : list
        List of timestamps to prepare tracks for.
    all_gnd : list
    ...
    if dataset.value in ['Sim1', 'Real']:
        ...
        for key, tracks in local_tracks.items():
            ...
            filtered_tracks[key] = {assoc.associations.pop().objects[0]}
        return fused_tracks, filtered_tracks
    else:
