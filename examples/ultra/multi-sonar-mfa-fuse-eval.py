...
from stonesoup.initiator.twostate import TwoStateInitiator, TwoStateInitiatorMixture
...
def run_sim(sim_iter):
    ...
    bias_tracker = MultiTargetMultiMixtureTracker(initiator, deleter, None, data_associator, updater)
...
def main():
    # for sim_iter in range(num_sims):
    #     run_sim(sim_iter)
    ...
    pickle.dump({'ospa': ospa_mat, 'gospa': gospa}, open('./output/mfa_metrics_full.pickle', 'wb'))
    ...
    plt.legend()
    pickle.dump({'ospa': ospa, 'gospa': gospa}, open('./output/mfa_metrics.pickle', 'wb'))
    plt.show()
