function [sensorData, colours] = getData2
...
%------
lonlat_min = min(d.elintdata.coords(:,1:2),[],1);
lonlat_max = max(d.elintdata.coords(:,1:2),[],1);
mmsi = [str2double(d.aisdata.mmsi); d.elintdata.mmsi];
[uqmmsi, n] = uniquewithcounts(mmsi);
[nmax, ii] = max(n);
d.aisdata = selectrows(d.aisdata, str2double(d.aisdata.mmsi)==uqmmsi(ii));
d.elintdata = selectrows(d.elintdata, d.elintdata.mmsi==uqmmsi(ii));
d.elintdata = rmfield(d.elintdata, 'mmsi');
d.aisdata = selectrows(d.aisdata,...
    d.aisdata.times > d.aisdata.times(1) + hours(6) &...
    d.aisdata.times < d.aisdata.times(1) + hours(18));
% ------
...
% 1 = 3GHz, 2 = 5GHz
radarType = ones(size(elintcolour,1),1); %(rand(size(elintcolour,1),1) > 0.5) + 1;
