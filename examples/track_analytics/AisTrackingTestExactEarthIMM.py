...
from stonesoup.initiator.simple import LinearMeasurementInitiatorMixture, SimpleMeasurementInitiator
...
from plotting import plot_map, plot_tracks, plot_cov_ellipse, plot_data
...
if __name__ == '__main__':
    ...
    for file_path in file_paths:
        ...
        # limits = np.deg2rad(np.array([[LON_MIN, LON_MAX], [LAT_MIN, LAT_MAX]]))
        # detector = BoundingBoxReducer(limits, [0, 1])
        ...
        for scan_time, detections in detector:
            ...
            # Filter by mmsi
            # mmsis =['271001057']
            # detections ={det for det in detections if det.metadata['MMSI'] in mmsis}
            ...
            if msvcrt.kbhit():
                ...
                if ch == b'f' or ch == b'p' or ch == b'm':
                    ...
                    plt.clf()
                    if SHOW_MAP:
                        plot_map(scan_time, lims, RES, TARGET)
                    plot_data(detections)
                    if ch == b'f':
                        plot_tracks(tracks, lims, RES, show_probs=False, show_map=SHOW_MAP, show_mmsis=True)
                        plt.pause(0.01)
                    elif ch == b'p':
                        plot_tracks(tracks, lims, RES, show_probs=True, show_map=SHOW_MAP, show_mmsis=False)
                        plt.show()
                    elif ch == b'm':
                        t_tracks = tracks
                        plot_tracks(t_tracks, lims, RES, show_mmsis=False, show_map=SHOW_MAP)
                        plt.show()
                elif ch == b'r':
