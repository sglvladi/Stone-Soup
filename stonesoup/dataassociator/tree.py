...
class LongLatTPRTreeMixIn(TPRTreeMixIn):
    ...
    offset: StateVector = Property(doc='2D vector specifying the offset. Default is ``np.array([[Longitude(0)], [Latitude(0)]])``.',
                                   default=np.array([[Longitude(0)], [Latitude(0)]]))
    window_delta: float = Property(doc='Confidence window added to detection windows when checking for +-180 overlaps. '
                                       'Default is 0.2rad',
                                   default=0.2)
    ...
    def generate_hypotheses(self, tracks, detections, timestamp, **kwargs):
        ...
        misdet = MissedDetection(timestamp=timestamp)
        mult = MultipleHypothesis()
        return {track: self.hypothesiser.hypothesise(
            track, track_detections[track], timestamp, missed_detection=misdet, mult=mult, **kwargs)
            for track in tracks}
